...
def main() -> None:
    ...
    assert template.mean is not None and template.tolerance is not None  # save() 已校验

    print(f"模板已保存：{output_path}")
    print(f"输入序列数：{template.n_videos}")
    print("特征\t均值\t容差")
    for idx, (mean, tolerance) in enumerate(zip(template.mean, template.tolerance, strict=True)):

...
- 107 项 Python 单元、CLI、API、输入校验和回归测试，其中 25 项为向量化 DTW 与旧逐单元实现的随机对拍（路径逐点一致），并含分段多阶段输出、逐帧时间轴、逐帧容差带、模板新旧格式回退、band 约束、评分权重解耦、语义阶段标签、特征降级剔除与权重钳制的直接测试。
...
- 29 项 vitest 前端测试覆盖 API 客户端、上传校验 hook 与各面板组件（含时间轴面板、语义阶段时间轴、容差带显示与骨架叠加），另有 React TypeScript 生产构建与 oxlint 检查。

...
- 从专家视频或预计算特征构建动作模板：专家序列 DTW 对齐后逐帧统计，产出逐帧参考曲线与随动作阶段变化的逐帧容差带（蹲底与站立位容差不同），低方差场景使用可配置的 5 度容差下限；旧格式模板自动回退为全局容差。1/std 特征权重带极差钳制，小样本模板不会被单特征主导。
- 实现标准 DTW 与方差反比加权的分段 DTW，可选 Sakoe-Chiba 带约束；分段不一致时自动回退到完整序列对齐。
- 输出总分、逐关节偏差、异常帧占比、峰值偏差与峰值时刻；异常帧按唯一测试帧统计，动作停顿不会重复计分；评分权重与对齐权重解耦（默认等权评分）。
- 深蹲各阶段按膝角轨迹标注语义标签（站立准备/下蹲/蹲底/起立/站立还原），非深蹲动作回退编号标签。
- 多次往复的视频按膝角曲线自动切分并逐次评分：总分为逐次均值，逐次摘要（得分、最低膝角、时间区间）随响应返回，分数不再随拍摄起止点波动。
- 响应携带逐帧关节角时间轴（实测曲线、对齐专家曲线、逐帧容差带、越界标记）、2D 骨架关键点通道、每个关节的容差与评分权重，评分依据可以从响应直接读出。
...
所有端点均带 Pydantic 响应模型，`http://127.0.0.1:8000/docs` 提供完整的字段类型。诊断响应中的每个异常项包含结构化关节标识（`joint`、`parent`、`child`、`joint_display`）与偏差方向，`phases` 为分段对齐真实检测到的多个阶段（含 `label` 语义标签）；分段失败回退到全序列对齐时，`segmented` 字段为 `false`。

检测到至少两次完整深蹲往复时，响应还会返回 `reps`（逐次评分摘要）与 `rep_detail_index`：`total_score` 为逐次均值，`phases`/`timeline` 详情对应代表次（时刻为原视频坐标）；单次输入行为不变。

诊断响应还包含：`timeline`（按测试帧对齐的逐帧关节角时间轴，含实测/专家曲线、逐帧容差带与越界标记，超长视频自动降采样）、`skeleton`（降采样 2D 骨架关键点、置信度与连接表，仅视频评估返回）、`per_feature_summary` 中每个关节的 `tolerance_deg` 与 `score_weight`、`excluded_features`（未参与评分的特征及原因）与 `advice_source`（建议实际来源）。
...
- 116 项 Python 单元、CLI、API、输入校验和回归测试，其中 25 项为向量化 DTW 与旧逐单元实现的随机对拍（路径逐点一致），并含分段多阶段输出、逐帧时间轴、逐帧容差带、往复检测与逐次评分、模板新旧格式回退、band 约束、评分权重解耦、语义阶段标签、特征降级剔除与权重钳制的直接测试。
...
- 30 项 vitest 前端测试覆盖 API 客户端、上传校验 hook 与各面板组件（含时间轴面板、语义阶段时间轴、逐次评分条、容差带显示与骨架叠加），另有 React TypeScript 生产构建与 oxlint 检查。

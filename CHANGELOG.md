...
主题：「逐次往复评分」。模板按单次深蹲构建，此前多次往复的视频被整段硬对齐到单次模板，往复间歇与站立都计为偏差，分数随拍摄起止点大幅波动（同一视频未裁剪与裁剪单次实测差 12.5 分）。现在按膝角曲线自动切分每次往复、逐次评分。

### 破坏性变更

- 检测到 ≥2 次完整往复时评分口径变化：`total_score` 为逐次评分的均值；`phases`/`timeline` 详情取分数最接近均值的代表次，时刻已偏移回原视频坐标。单次或未检测到往复的输入行为完全不变。
- 响应新增 `reps`（逐次摘要：`index`/`time_range`/`score`/`knee_min_deg`）与 `rep_detail_index`（详情对应第几次），仅逐次评分时返回。

### 新增

- `movescope.reps`：膝角滞回往复检测——阈值取 5/95 百分位抗单帧尖刺，最短时长（0.5 秒）滤掉首尾残缺片段，边缘复制平滑避免零填充制造假谷底，未回到站立位的末尾残段不计入；API、Gradio 与 CLI（`movescope-assess-features`）三条评估链路统一接入；非深蹲动作不套用膝角切分。
- Web 工作台总分下方新增逐次评分条：每次往复的得分与最低膝角，点击跳转视频对应时刻，高亮详情面板展示的那次；Gradio 文本摘要新增逐次评分行。

### 测试

- Python 测试从 107 项扩展到 116 项：往复检测（多往复、浅动作、瞬时尖刺、末尾残段）、逐次均值与详情时间偏移、单次回退、非深蹲跳过、膝角 NaN 回退、多往复 API 端到端。
- 前端 vitest 从 29 项扩展到 30 项：逐次评分条渲染、点击跳转与详情高亮。

## 0.5.0 - 2026-07-27

主题：「逐帧口径的模板与评分」。容差与参考曲线从「整段一个数」升级为随动作阶段变化的逐帧曲线，评分权重与对齐权重解耦。

### 破坏性变更

- 模板统计升级为逐帧口径（格式 v2）：构建时把每条专家序列用无权重 DTW 对齐到代表序列，产出逐帧参考曲线 `reference_seq` 与形状 `(T_ref, D)` 的逐帧容差带 `tolerance_band`——蹲底与站立位不再共用同一个容差；全局 `std`/`tolerance` 改为逐帧统计的时间平均。旧格式（v1）npz 模板仍可加载，自动回退为代表序列 + 全局容差广播；重建模板即可获得逐帧容差。分数会与 0.4.0 有漂移。
- 评分权重与对齐权重解耦：`AssessmentEngine.score_weights` 默认从 `"aligner"` 改为 `"uniform"`——对齐仍用 1/std 特征权重，评分等权，低方差特征不再被对齐与评分双重放大。显式传 `score_weights="aligner"` 可恢复旧口径。这同样引入分数漂移。
- API 契约：`timeline.series[].tolerance_deg` 由标量变为与 `test_deg` 等长的逐帧数组（旧模板由引擎广播为等值数组）；`phases[]` 新增 `label` 语义标签字段。

### 新增

- DTW 支持 Sakoe-Chiba 带约束（`band_ratio`，默认关闭）：把对齐路径限制在斜率归一对角线附近，带宽自动放宽到不小于两序列长度差，保证始终存在可行路径。
- 深蹲阶段语义标签：分段评估的各阶段按膝角轨迹标注「站立准备/下蹲/蹲底/起立/站立还原」；非深蹲动作或膝角活动范围不足 15 度时回退编号标签，不对未知动作强行套用深蹲语义。Web 阶段时间轴、主要问题列表与 Gradio 文本摘要均展示标签。
- Web 时间轴的容差走廊随逐帧容差带起伏，底部统计显示容差范围（恒定时显示单值，变化时显示区间）。

### 测试

- Python 测试从 92 项扩展到 107 项：新增模板 v2 逐帧曲线与新旧格式回退、band 约束可行性与参数校验、逐帧容差带驱动越界、评分权重解耦、语义阶段标签及回退等覆盖。
- 前端 vitest 从 28 项扩展到 29 项：语义阶段标签渲染、容差带区间与恒定容差显示。
